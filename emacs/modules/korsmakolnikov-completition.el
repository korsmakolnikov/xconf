(upsert-package 'cape)
(upsert-package 'consult)
(upsert-package 'corfu)
(upsert-package 'corfu-terminal)
(upsert-package 'embark)
(upsert-package 'embark-consult)
(upsert-package 'marginalia)
(upsert-package 'orderless)
(upsert-package 'vertico)
...
(when (require 'vertico nil :noerror)
  ...
  ;; they have been turned on by dalimacs-defaults-config, because they interfere
  ;; with this module.
  ; (with-eval-after-load 'dalimacs-defaults-config
  ;   (fido-mode -1)
...
; (dalimacs/prepare-keymap
;  (dalimacs/main-menu-definer
;   "bb"
...
(when (require 'embark nil :noerror)
  ...
  (keymap-global-set "C-E" 'embark-act)
  (keymap-global-set "M-E" 'embark-dwim)
...
(when (require 'corfu nil :noerror)
  ...
  (when (require 'corfu-popupinfo nil :noerror)
    ...
    (keymap-set corfu-map "C-M-SPC" #'corfu-insert-separator)
...
(when (require 'cape nil :noerror)
  ...
  ; (defun dalimacs-completion-corfu-eshell ()
  ...
  ;    corfu-auto nil)
  ;   (corfu-mode))
  ; (add-hook 'eshell-mode-hook #'dalimacs-completion-corfu-eshell))
  )

(provide 'korsmakolnikov-completition)
